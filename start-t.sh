...
[[ $EUID != 0 ]] && echo -e "$Error 当前账号非ROOT(或没有ROOT权限)，无法继续操作，请使用$Green_background_prefix sudo su $Font_color_suffix来获取临时ROOT权限（执行后会提示输入当前账号的密码）。" && exit 1
...
#Install_v2ray
Install_v2ray(){
	wget -qN --no-check-certificate https://github.com/johnpoint/One-step-to-V2ray/raw/master/v2ray.sh && chmod +x v2ray.sh && ./v2ray.sh
}
...
=$Green_font_prefix位置：$Font_color_suffix$country $region $city
=$Green_font_prefix经纬：$Font_color_suffix$loc
=$Green_font_prefix组织：$Font_color_suffix$org
=============== System Information ===================
=	$time
